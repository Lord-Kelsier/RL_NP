...
def checkFiles():
  def createFile():
    with open(f"{FOLDER}/Multi-epsilon={EPSILON}.json", "w") as f:
      MaxReward = {
        "reward": None,
        "positions": None,
        "time": time.time(),
  ...
  if FOLDER not in os.listdir():
    os.mkdir(FOLDER)
  if f"Multi-epsilon={EPSILON}.json" not in os.listdir(FOLDER):
    createFile()
...
def saveMaxReward(reward, observation, step):
  ...
  if observation[0][-1][0] == 0:
    return
  with open(f"{FOLDER}/Multi-epsilon={EPSILON}.json", "r") as f:
